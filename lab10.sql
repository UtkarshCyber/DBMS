 CREATE TABLE STUDENT ( USN CHAR (10) PRIMARY KEY, SNAME CHAR (25), ADDRESS CHAR (25), PHONE INT, GENDER CHAR (1))
CREATE TABLE SEMSEC (SSID CHAR (5) PRIMARY KEY, SEM INT, SEC CHAR (1))
CREATE TABLE CLASS ( USN CHAR (10), SSID CHAR (5), PRIMARY KEY (USN, SSID), FOREIGN KEY (USN) REFERENCES STUDENT (USN), FOREIGN KEY (SSID) REFERENCES SEMSEC (SSID))
CREATE TABLE SUBJECT ( SUBCODE CHAR (8), TITLE CHAR (20), SEM INT , CREDITS INT , PRIMARY KEY (SUBCODE))
CREATE TABLE IAMARKS ( USN CHAR (10), SUBCODE CHAR (8), SSID CHAR(5), TEST1 INT, TEST2 INT, TEST3 INT, FINALIA INT, PRIMARY KEY (USN, SUBCODE, SSID), FOREIGN KEY (USN) REFERENCES STUDENT (USN), FOREIGN KEY (SUBCODE) REFERENCES SUBJECT (SUBCODE), FOREIGN KEY (SSID) REFERENCES SEMSEC (SSID))
...
4.	Categorize students based on the followingcriterion: If FinalIA = 17 to 20 then CAT =‘Outstanding’ If FinalIA = 12 to 16 then CAT = ‘Average’ If FinalIA< 12 then CAT = ‘Weak’ Give these details only for 8th semester A, B, and C section students.
